...
// Gnathus 66467
struct npc_gnathus : public ScriptedAI
{
    npc_gnathus(Creature* creature) : ScriptedAI(creature) { }
        ...
        { 
            ...
            if (player->GetQuestObjectiveCounter(OBJECTIVE_SIKTHIK_CAGES_SEARCHED) == 7 && player->GetQuestStatus(QUEST_THE_SEARCH_OF_RESTLESS_LENG) == QUEST_STATUS_INCOMPLETE)
            {
                if (Creature* leng = player->SummonCreature(NPC_RESTLESS_LENG, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 20000))
                {
                    ...
                    leng->GetScheduler().Schedule(Seconds(2), [leng](TaskContext /*context*/)             { leng->AI()->Talk(0); });
                    leng->GetScheduler().Schedule(Seconds(2), [leng](TaskContext /*context*/)             { leng->SetStandState(UNIT_STAND_STATE_STAND); });
                    leng->GetScheduler().Schedule(Seconds(3), [leng](TaskContext /*context*/)             { leng->AI()->Talk(1); });
                    leng->GetScheduler().Schedule(Seconds(6), [leng, playerGuid](TaskContext /*context*/) { if (Player* player = ObjectAccessor::GetPlayer(*leng, playerGuid)) leng->AI()->Talk(2, player); });
                }
                player->KilledMonsterCredit(NPC_RESTLESS_LENG);
            }

            ObjectGuid goGuid = go->GetGUID();
            player->GetScheduler().Schedule(Seconds(8), [player, goGuid](TaskContext /*context*/)
            {/*
        ...
        bool OnGossipHello(Player* player, GameObject* go) override
        {
                ...
                && player->GetQuestObjectiveCounter(QUEST_OBJECTIVE_LONGYIN_RANGER_RESCUED) < 4)
            {
                if (auto ranger = GetClosestCreatureWithEntry(player, NPC_LONGYING_RANGER, 10.f))
                {
                    ...
                    player->GetQuestObjectiveData(QUEST_OBJECTIVE_LONGYIN_RANGER_RESCUED, 1);
    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            if (player->GetQuestStatus(QUEST_RANGER_RESCUE) == QUEST_STATUS_INCOMPLETE
                ...
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Examine the body.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
        {
            ...
            {
                player->GetQuestObjectiveData(QUEST_OBJECTIVE_FREE_LIN_SILENTSTRIKE, 1);
            ...
            void JustDied(Unit* killer) override 
            {
                if (Unit* Torch = ObjectAccessor::GetUnit(*me, TorchGUID))
                {
                    if (killer && killer->ToPlayer() && killer->ToPlayer()->GetQuestStatus(QUEST_THE_TORCHES) == QUEST_STATUS_INCOMPLETE)
                        Torch->AddNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
        ...
        struct npc_suna_silentstrikeAI : public ScriptedAI
        {
            ...
            void DamageTaken(Unit* who, uint32& damage) override
            {
                if (damage >= me->GetHealth())
                {
                    ...
                    AddTimedDelayedOperation(5000, [this, suna]() -> void { suna->SetStandState(UNIT_STAND_STATE_STAND); });
                    AddTimedDelayedOperation(5000, [this, suna]() -> void { suna->setDeathState(JUST_DIED);              });
                    AddTimedDelayedOperation(5000, [this, suna]() -> void { suna->DespawnOrUnsummon();                   });
                }
            }
...
public:
    ...
    bool OnConditionCheck(Condition const*  condition , ConditionSourceInfo& source)
    {
...
void AddSC_townlong_steppes()
{
    ...
    new npc_lith_ik();
    new npc_eshelon();
    RegisterCreatureAI(npc_lon_the_bull);
    RegisterCreatureAI(npc_norlaxx);
    RegisterCreatureAI(npc_siltriss_sharpener);
    RegisterCreatureAI(npc_yul_wildpaw);
    // Elite mobs
    new npc_darkwoods_faerie();
    new npc_hei_feng();
    // Standard Mobs
    RegisterCreatureAI(npc_seething_flashripper);
    RegisterCreatureAI(npc_korthik_timberhusk);
    RegisterCreatureAI(npc_rankbite_ancient);
    RegisterCreatureAI(npc_deadtalker_crusher);
    RegisterCreatureAI(npc_longshadow_mushan);
    RegisterCreatureAI(npc_seething_hatred);
    RegisterCreatureAI(npc_gnathus);
    // Quests
    ...
    RegisterCreatureAI(npc_townlong_gunpowder_cask);

    new go_sikthik_cage();
    new go_drywood_cage();

    RegisterSpellScript(spell_gather_steam);
    RegisterSpellScript(spell_q30959);
    RegisterSpellScript(spell_townlong_shado_pan_spike_trap_eff);
    RegisterAuraScript(spell_protective_shell);
