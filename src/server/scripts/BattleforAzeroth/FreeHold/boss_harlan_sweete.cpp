...
struct boss_harlan_sweete : public BossAI
{
    ...
    {
        _JustReachedHome();
        instance->SetBossState(DataHarlanSweete, FAIL);
    }
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        events.ScheduleEvent(EventCannonBarrage, 8000);
        events.ScheduleEvent(EventSwiftwindSaber, 12000);
        events.ScheduleEvent(EventSummonIrontideGrenadier, 15000);
    }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        _JustDied();
        Talk(TalkDead);
        instance->SetBossState(DataHarlanSweete, DONE);
    }
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage) override
    {
        if (me->HealthWillBeBelowPctDamaged(60, damage) && !sixtyPercent)
        {
            sixtyPercent = true;
            Talk(Talk60Percent);
            me->CastSpell(me, LoadedDiceAllHands, true);
        }
        else if (me->HealthWillBeBelowPctDamaged(30, damage) && !thirtyPercent)
        {
            thirtyPercent = true;
            Talk(Talk30Percent);
            me->CastSpell(me, LoadedDiceManOWar, true);
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EventCannonBarrage:
            {
                ...
                {
                    Talk(TalkCannon);
                    me->CastSpell(target, CannonBarrage, false);

                    if (me->HasAura(LoadedDiceAllHands))
                    {
                        ...
                        if (!playerList.empty())
                        {
                            playerList.remove_if([](Player * player) -> bool
                                {
                                    if (player->HasAura(CannonBarrage))
                                        return true;
                            ...
                            if (Player * secondTarget = Trinity::Containers::SelectRandomContainerElement(playerList))
                                me->CastSpell(secondTarget, CannonBarrage, true);
                        }
                    }
                }
                if (!me->HasAura(LoadedDiceManOWar))
                    events.Repeat(12000);
                else
                    events.Repeat(8000);
                break;
            }
            case EventSwiftwindSaber:
            {
                me->CastSpell(me, SwiftwindSaber, false);
            ...
            case EventSummonIrontideGrenadier:
            {
                Talk(TalkGranadier);

                me->SummonCreature(NpcIrontideGrenadier, GranadierSummonPos, TEMPSUMMON_MANUAL_DESPAWN);
...
struct npc_irontide_granadier : public ScriptedAI
{
        ...
        AddTimedDelayedOperation(800, [this]() -> void
            {
                ...
                events.ScheduleEvent(EventCheckPlayer, 500);
            });
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            {
                if (!me->HasAura(BlackPowderBombAura))
                    me->CastSpell(me, BlackPowderBomb, false);
    ...
    void OnPeriodic(AuraEffect const* aurEff)
    {
        if (Unit * caster = GetCaster())
            if (Unit * target = GetTarget())
                caster->CastSpell(target, FlamingShrapnel, true);
    }
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        if (Unit * caster = GetCaster())
        {
            if (Unit * target = GetHitUnit())
            {
                ...
                caster->GetCreatureListWithEntryInGrid(cannonList, NpcIrontideCannon, 200.0f);
                if (!cannonList.empty())
                {
                    if (Creature * cannon = Trinity::Containers::SelectRandomContainerElement(cannonList))
                    {
                        cannon->CastSpell(target, CannonBarrageMissile, true);
                    }
                }
            }
        }
...
class spell_swiftwind_saber : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        {
            uint8 count = 3;
            caster->CastSpell(caster, SwiftwindSaberSummon, true);

            if (caster->HasAura(LoadedDiceAllHands))
                count = 6;

            for (uint8 i = 0; i < count; i++)
                caster->SummonCreature(NpcSwiftwindSaber, caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ(), caster->GetOrientation() - (float(M_PI) / 6.0f) * (i), TEMPSUMMON_TIMED_DESPAWN, 5000);
        }
    }
    ...
    void OnUnitEnter(Unit* unit)
    {
        if (Unit * caster = at->GetCaster())
            if (Player * player = unit->ToPlayer())
                if (!player->HasAura(FieryDebris))
                    caster->CastSpell(player, FieryDebris, true);
    }
        ...
        if (Unit * caster = at->GetCaster())
        {
            ...
            for (ObjectGuid guid : insideUnits)
                if (Player * player = ObjectAccessor::GetPlayer(*caster, guid))
                    player->CastSpell(player, SwiftwindSaberDmg, true);
        }
    }
};
