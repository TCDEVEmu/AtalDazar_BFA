...
struct scenario_draenor_shadowmoon_valley : public InstanceScript
{
    scenario_draenor_shadowmoon_valley(InstanceMap* map) : InstanceScript(map) { }
    ...
    void OnPlayerEnter(Player* player) override
    {
        ...
        {
            //player->RemovePhase(PHASE_THE_FATE_OF_KARABOR);
            //player->RemovePhase(PHASE_THE_RIGHTEOUS_MARCH);

            //player->SetPhaseId(Phase, true);
        });
        events.ScheduleEvent(DATA_FOLLOW_PROPHET_VELEN, 5000);
    }
    ...
    void OnUnitDeath(Unit* unit)
    {
        ...
        case NPC_KRULL_79560:
            ...
            if (Creature* velen = GetCreature(NPC_PROPHET_VELEN_79522))
            {
                velen->GetScheduler().Schedule(20s, [this, velen](TaskContext /*context*/)
                {
                    ...
                    velen->SetNpcFlags(UNIT_NPC_FLAG_GOSSIP);
                });
            }
        ...
        case NPC_COMMANDER_VORKA_74715:
        {
            ...
            {
                yudian->SetNpcFlags(UNIT_NPC_FLAG_SPELLCLICK);
...
struct npc_prophet_velen_79522 : public ScriptedAI
{
    ...
    {
        events.ScheduleEvent(SPELL_HOLY_SMITE, 4000);
    }
...
struct npc_yrel_79521 : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        events.ScheduleEvent(SPELL_SHIELD_OF_VENGEANCE, 5000);
    }
...
struct npc_krull_79560 : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        events.ScheduleEvent(SPELL_BRUTAL_SMASH, 5000);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        {
            events.ScheduleEvent(SPELL_LEAP, 3000);
...
struct npc_commander_vorka_74023 : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        events.ScheduleEvent(SPELL_FIRE_SUPPORT, 10000);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case SPELL_FIRE_SUPPORT:
            ...
            events.ScheduleEvent(SPELL_FIRE_SUPPORT, 20000);
            break;
        }
        DoMeleeAttackIfReady();
    }
...
public:
    ...
    void OnSceneTriggerEvent(Player* player, uint32 p_SceneInstanceID, std::string triggerName) override
    {
        ...
        if (triggerName == "Teleport")
        {
            ...
            {
                //if (player->HasQuest(QUEST_THE_RIGHTEOUS_MARCH) || player->GetQuestStatus(QUEST_THE_RIGHTEOUS_MARCH) == QUEST_STATUS_REWARDED)
                //    player->SetPhaseId(PHASE_THE_RIGHTEOUS_MARCH, true);
            });
        }
    }
};
...
struct npc_arnokk_the_burner_75358 : public ScriptedAI
{
    ...
    {
        Talk(0);
        events.ScheduleEvent(SPELL_BURN, 5000);
        events.ScheduleEvent(SPELL_FIERY_BLAST, 10000);
        events.ScheduleEvent(SPELL_SCORCHED_EARTH, 15000);
    }
...
struct npc_yrel_73395 : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        events.ScheduleEvent(SPELL_CONSECRATION, 5000);
        events.ScheduleEvent(SPELL_DIVINE_STORM, 5000);
        events.ScheduleEvent(SPELL_HAMMER_OF_JUSTICE, 5000);
        events.ScheduleEvent(SPELL_HAMMER_OF_RIGHTEOUS, 5000);
    }
            ...
            if (me->IsWithinDist(player, 15.0f, false))
            {
                if (instance)
                {
                    if (!IsLock && player->HasQuest(QUEST_THE_DEFENSE_OF_KARABOR))
                    {
                        ...
                        player->CompletedCriteriaTreeId(CRITERIA_TREE_FIGHT_YOUR_WAY_TO_THE_KARABOR_HARBOR);
...
struct npc_commander_vorka_74715 : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/) override
    {
        Talk(4);
        Talk(2);
        events.ScheduleEvent(SPELL_BLAZING_SHOT, 5000);
        events.ScheduleEvent(SPELL_FIRE_SUPPORT_BLACK_SKIES, 10000);
        events.ScheduleEvent(SPELL_FIRE_SUPPORT_BLACKEST_SKIES, 25000);
        events.ScheduleEvent(SPELL_GRAPESHOT_BLAST, 5000);
    }
...
{
    RegisterInstanceScript(scenario_draenor_shadowmoon_valley, 1277);
