...
void HandleInitCombat(Unit* me  )
{
    ...
    if (pList.size() > 1)
    {
        Trinity::Containers::RandomResize(pList, 1);
...
class spell_galleon_fire_shot : public SpellScript
{
    ...
    {
        if (targets.size() > 3)
            Trinity::Containers::RandomResize(targets, (int)(targets.size() / 2));
    }
