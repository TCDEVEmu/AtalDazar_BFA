...
/*
// curent range to pos
float GetCirclePlatformPath(ObjectGuid casterGUID, Position pos)
{
...
Position GetPlatformPathPoint(float m_ori, ObjectGuid casterGUID, float m_dist = 40.0f)
{
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = nonCombatEvents.ExecuteEvent())
                {
                    if (eventId == EVENT_MOVE)
                    {
                        ...
                        if (Player* pItr = me->SelectNearestPlayer(60.0f))
                        {
                            me->Attack(pItr, true);
    ...
    public:
        ...
        class spell_prey_time_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex effIndex)
            {
                ...
                target->CastSpell(caster, GetSpellInfo()->GetEffect(EFFECT_1)->BasePoints, false);
            }
