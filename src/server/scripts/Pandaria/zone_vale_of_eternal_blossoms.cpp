...
// Dagou 59977
struct npc_dagou : public ScriptedAI
{
    npc_dagou(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_mercurial_guardian : public ScriptedAI
{
    npc_mercurial_guardian(Creature* creature) : ScriptedAI(creature) { }
...
class spell_nimbus_shroud : public AuraScript
{
    ...
    void OnAuraEffectRemove(AuraEffect const* aurEff, AuraEffectHandleModes mode)
    {
        if (Creature* owner = GetOwner()->ToCreature())
            owner->RemoveUnitFlag(UnitFlags(UNIT_FLAG_PACIFIED | UNIT_FLAG_NON_ATTACKABLE));
    }

    void OnAuraEffectApply(AuraEffect const* aurEff, AuraEffectHandleModes mode)
    {
        if (Creature* owner = GetOwner()->ToCreature())
            owner->AddUnitFlag(UnitFlags(UNIT_FLAG_PACIFIED | UNIT_FLAG_NON_ATTACKABLE));
    }
...
void AddSC_vale_of_eternal_blossoms()
{
    ...
    RegisterCreatureAI(npc_alani);
    RegisterCreatureAI(npc_shao_tien_dominator);
    RegisterCreatureAI(npc_gochao_the_iron_fist);
    RegisterCreatureAI(npc_bloodtip);
    RegisterCreatureAI(npc_mogu_jade_guardian);
    RegisterCreatureAI(npc_wulon);
    RegisterCreatureAI(npc_milau);
    RegisterCreatureAI(npc_ancient_mogu_spirit);
    RegisterCreatureAI(npc_aetha_tornado);
    RegisterCreatureAI(npc_thundermaw);
    RegisterCreatureAI(npc_shao_tien_fist);
    RegisterCreatureAI(npc_shao_tien_antiquator);
    RegisterCreatureAI(npc_kang_the_soul_thief);
    RegisterCreatureAI(npc_shao_tien_soul_caller);
    RegisterCreatureAI(npc_dagou);
    RegisterCreatureAI(npc_mercurial_guardian);
    RegisterCreatureAI(npc_aetha);
    RegisterCreatureAI(npc_quid);
    RegisterCreatureAI(npc_yorik_sharpeye);
    RegisterCreatureAI(npc_general_temuja);
    RegisterCreatureAI(npc_veb_shado_pan_trainee);

    RegisterAuraScript(spell_nimbus_shroud);
    RegisterAuraScript(spell_icy_destruction);
    RegisterSpellScript(spell_alani_remove_protections);
    RegisterSpellScript(spell_devastating_leap);
    RegisterSpellScript(spell_crush_uprising);
