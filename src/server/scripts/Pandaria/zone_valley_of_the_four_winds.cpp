    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            if (creature->GetEntry() == NPC_WEI_BLACKSOIL && creature->AI()->GetData(DATA_AMBUSHED) == 0)
                SendGossipMenuFor(player, 18832, creature->GetGUID());
            else if (creature->GetEntry() == NPC_WEI_BLACKSOIL && creature->AI()->GetData(DATA_AMBUSHED) == 1)
                SendGossipMenuFor(player, 18833, creature->GetGUID());
            else
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
        {
            ...
            if (action == GOSSIP_ACTION_INFO_DEF + 1)
            {
                ...
                switch (creature->GetEntry())
                {
                    case NPC_WEI_BLACKSOIL:
                    {
                        ...
                        wei->AI()->Talk(0);
                        wei->GetScheduler().Schedule(Seconds(3), [wei, playerGuid](TaskContext /*context*/)
                        {
                            wei->AI()->Talk(1);
                    ...
                    case NPC_SHU_LI_SPADEPAW:
                    {
                        ...
                        shuLi->AI()->Talk(0);
                        shuLi->GetScheduler().Schedule(Seconds(5), [shuLi](TaskContext /*context*/) { shuLi->AI()->Talk(1); });
                        feng->GetScheduler().Schedule(Seconds(5), [feng, playerGuid](TaskContext /*context*/) { if (Player * player = ObjectAccessor::GetPlayer(*feng, playerGuid)) feng->AI()->Talk(0, player); });
                        break;
                    }
                    ...
                    case NPC_YING_GRENNTILL:
                    {
                        ...
                        haiyunSum->GetScheduler().Schedule(Seconds(3), [haiyunSum, huizhongSumGuid, yingSumGuid](TaskContext /*context*/)
                        {
                            haiyunSum->AI()->Talk(1);
                        ...
                        yingSum->GetScheduler().Schedule(Seconds(3), [yingSum](TaskContext /*context*/)
                        {
                            yingSum->AI()->Talk(0);
                        });
                        huizhongSum->GetScheduler().Schedule(Seconds(3), [huizhongSum, playerGuid](TaskContext /*context*/)
                        {
                            if (Player* player = ObjectAccessor::GetPlayer(*huizhongSum, playerGuid))
                                huizhongSum->AI()->Talk(0, player);
                        });
                        haiyunSum->GetScheduler().Schedule(Seconds(3), [haiyunSum](TaskContext /*context*/)
                        {
                            haiyunSum->AI()->Talk(2);
                        });
                        haiyunSum->GetScheduler().Schedule(Seconds(3), [haiyunSum, playerGuid](TaskContext /*context*/)
                        {
                            if (Player* player = ObjectAccessor::GetPlayer(*haiyunSum, playerGuid))
                                haiyunSum->SetFacingToObject(player);
                            haiyunSum->AI()->Talk(3);
                        });
                        haiyunSum->GetScheduler().Schedule(Seconds(3), [haiyunSum, yingSumGuid](TaskContext /*context*/)
                        {
                        ...
                        huizhongSum->GetScheduler().Schedule(Seconds(1), [huizhongSum](TaskContext /*context*/)
                        {
                            huizhongSum->GetMotionMaster()->MovePoint(0, -366.897f, 1996.729f, 125.218f);
                        });
                        huizhongSum->GetScheduler().Schedule(Seconds(10), [haiyunSumGuid, yingSumGuid, huizhongSum, yingGuid, haiyunGuid, huizhongGuid](TaskContext /*context*/)
                        {
                    ...
                    case NPC_MIA_MARLFUR:
                    {
                        ...
                        zhangSum->AI()->Talk(0);
                        zhangSum->GetScheduler().Schedule(Seconds(5), [zhangSum, playerGuid](TaskContext /*context*/)
                        {
                        ...
                        zhangSum->GetScheduler().Schedule(Seconds(3), [zhangSum](TaskContext /*context*/)
                        {
                            zhangSum->GetMotionMaster()->MovePoint(0, -335.536f, 2069.130f, 119.780f);
                            zhangSum->AI()->Talk(2);
                        });
                        zhangSum->GetScheduler().Schedule(Seconds(3), [zhangSum, playerGuid](TaskContext /*context*/)
                        {
                            zhangSum->AI()->Talk(3);
                            if (Player* player = ObjectAccessor::GetPlayer(*zhangSum, playerGuid))
                                zhangSum->SetFacingToObject(player);
                        ...
                        zhangSum->GetScheduler().Schedule(Seconds(1), [zhangSum, zhang](TaskContext /*context*/)
                        {
                            zhangSum->GetMotionMaster()->MovePoint(0, zhang->GetPosition());
                        });
                        zhangSum->GetScheduler().Schedule(Seconds(2), [zhangSum, zhang](TaskContext /*context*/)
                        {
            ...
            else if (action == GOSSIP_ACTION_INFO_DEF + 2)
            {
                ...
                weiSum->GetScheduler().Schedule(Seconds(3), [weiSum](TaskContext /*context*/) { weiSum->GetMotionMaster()->MovePoint(0, -326.190f, 2393.927f, 140.406f); });
                weiSum->GetScheduler().Schedule(Seconds(3), [weiSum](TaskContext /*context*/) { weiSum->GetMotionMaster()->MovePoint(0, -309.222f, 2400.113f, 140.223f); });
                weiSum->GetScheduler().Schedule(Seconds(3), [weiSum](TaskContext /*context*/) { weiSum->DespawnOrUnsummon(); });
                creature->GetScheduler().Schedule(Seconds(10), [creature](TaskContext /*context*/) { creature->SetVisible(true); creature->AI()->SetData(DATA_AMBUSHED, 0); });
            }
...
struct npc_liang_thunderfoot : public ScriptedAI
{
    ...
    void JustSummoned(Creature* summoned) override
    {
        ...
        AddTimedDelayedOperation(1000, [this, summoned, guid]() -> void
        {
...
struct npc_vfw_adolescent_mushan : public ScriptedAI
{
    npc_vfw_adolescent_mushan(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_wyrmhorn_turtle : public ScriptedAI
{
    npc_vfw_wyrmhorn_turtle(Creature* creature) : ScriptedAI(creature) { }
...
// Lupello 56357
struct npc_vfw_lupello : public ScriptedAI
{
    npc_vfw_lupello(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_snagtooth_hooligan : public ScriptedAI
{
    npc_vfw_snagtooth_hooligan(Creature* creature) : ScriptedAI(creature) { }
...
// Frenzyhop 56514
struct npc_vfw_frenzyhop : public ScriptedAI
{
    npc_vfw_frenzyhop(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_hornbill_strider : public ScriptedAI
{
    npc_vfw_hornbill_strider(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_grainhunter_hawk : public ScriptedAI
{
    npc_vfw_grainhunter_hawk(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_springtail_gnasher : public ScriptedAI
{
    npc_vfw_springtail_gnasher(Creature* creature) : ScriptedAI(creature) { }
...
// Mothfighter 57648
struct npc_vfw_mothfighter : public ScriptedAI
{
    npc_vfw_mothfighter(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_wildscale_herbalist : public ScriptedAI
{
    npc_vfw_wildscale_herbalist(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_ikthik_vanguard : public ScriptedAI
{
    npc_vfw_ikthik_vanguard(Creature* creature) : ScriptedAI(creature) { }
...
// Darkhide 58435
struct npc_vfw_darkhide : public ScriptedAI
{
    npc_vfw_darkhide(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_ik_thik_wing_commander : public ScriptedAI
{
    npc_vfw_ik_thik_wing_commander(Creature* creature) : ScriptedAI(creature), summons(me) { }
...
struct npc_vfw_manifestation_of_fear : public ScriptedAI
{
    npc_vfw_manifestation_of_fear(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_krungko_fingerlicker : public ScriptedAI
{
    npc_vfw_krungko_fingerlicker(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_vfw_unbarreled_pandaren : public ScriptedAI
{
    npc_vfw_unbarreled_pandaren(Creature* creature) : ScriptedAI(creature) { }
...
void AddSC_valley_of_the_four_winds()
{
    ...
    RegisterCreatureAI(npc_fish_fellreed);
    RegisterCreatureAI(npc_liang_thunderfoot);
    RegisterCreatureAI(npc_francis_the_shepherd_boy);
    RegisterCreatureAI(npc_just_a_folk_story_quest);
    RegisterCreatureAI(npc_granary_vandal);
    RegisterCreatureAI(npc_catch_and_carry);
    RegisterCreatureAI(npc_vfw_adolescent_mushan);
    RegisterCreatureAI(npc_vfw_wyrmhorn_turtle);
    RegisterCreatureAI(npc_vfw_lupello);
    RegisterCreatureAI(npc_vfw_snagtooth_hooligan);
    RegisterCreatureAI(npc_vfw_frenzyhop);
    RegisterCreatureAI(npc_vfw_hornbill_strider);
    RegisterCreatureAI(npc_vfw_grainhunter_hawk);
    RegisterCreatureAI(npc_vfw_springtail_gnasher);
    RegisterCreatureAI(npc_vfw_mothfighter);
    RegisterCreatureAI(npc_vfw_wildscale_herbalist);
    RegisterCreatureAI(npc_vfw_ikthik_vanguard);
    RegisterCreatureAI(npc_vfw_darkhide);
    RegisterCreatureAI(npc_vfw_barrel_of_fireworks);
    RegisterCreatureAI(npc_vfw_ik_thik_wing_commander);
    RegisterCreatureAI(npc_vfw_manifestation_of_fear);
    RegisterCreatureAI(npc_vfw_krungko_fingerlicker);
    RegisterCreatureAI(npc_vfw_unbarreled_pandaren);

    RegisterSpellScript(spell_vfw_prey_pounce);
    RegisterSpellScript(spell_vfw_apply_mask);
    RegisterSpellScript(spell_suspicious_footprint_identified);
    RegisterSpellScript(spell_four_wind_pick_up_chicken);
    RegisterSpellScript(spell_to_be_a_master_food_coocking);
    //RegisterSpellScript(spell_throw_grain);

    RegisterAuraScript(spell_vfw_krungko_timer);
    RegisterAuraScript(spell_vfw_breaking_barrel);
    RegisterAuraScript(spell_catch_grain);
