...
class npc_kraxik_tornado : public CreatureScript
{
    public:
        ...
        struct npc_kraxik_tornadoAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                //me->SetDisplayId(me->GetCreatureTemplate()->Modelid);
...
struct npc_shade_of_ling_heartfist : public ScriptedAI
{
    ...
    void JustSummoned(Creature* summon) override
    {
        summon->AddUnitFlag(UNIT_FLAG_PACIFIED);
    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            if (player->GetQuestStatus(29579) == QUEST_STATUS_INCOMPLETE)
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Get to Hanae's house. It's safe there.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        struct npc_instructor_skythornAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                if (eventPhase)
                {
                    if (eventTimer < diff)
                    {
                        Talk(eventPhase, ObjectAccessor::GetPlayer(*me, playerGUID));
                        eventTimer = 8000;
                        ++eventPhase;
                        if (eventPhase == 4)
                        {
    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            if (player->GetQuestStatus(29883) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, gossip, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            }
...
class AreaTrigger_q29586 : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger) override
        {
            ...
            {
                gormaliStalker1->AddUnitFlag(UnitFlags(UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE));
                ...
                gormaliStalker1->m_Events.Schedule(delay += 1000, [gormaliStalker1, playerGuid]()
                {
                    ...
                    gormaliStalker1->RemoveUnitFlag(UnitFlags(UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE));
            ...
            {
                gormaliStalker2->AddUnitFlag(UnitFlags(UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE));
                ...
                gormaliStalker2->m_Events.Schedule(delay += 1000, [gormaliStalker2, playerGuid]()
                {
                    ...
                    gormaliStalker2->RemoveUnitFlag(UnitFlags(UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE));
            ...
            static void SpawnWave(Creature* myang, uint64 casterguid, uint8 combatantCount, uint8 bruteCount)
            {
                ...
                if (player && player->IsAlive())
                {
                    for (uint8 j = 0; j < 2; j++)
                    {
                        ...
                        {
                            Position pos;
                            player->GetRandomNearPosition(15.0f);
        ...
        struct npc_rumpus_spawnAI : public ScriptedAI
        {
            ...
            void DamageTaken(Unit* /*who*/, uint32& damage) override
            {
                if (damage >= me->GetHealth())
                {
                    ...
                    AddTimedDelayedOperation(3000, [this]() -> void { me->GetMotionMaster()->MovePoint(0, rumpusEndPoint, true); });
                }
            }
            ...
            void Reset() override
            {
                ...
                switch (me->GetEntry())
                {
                    case NPC_WOUNDED_PILOT:
                        ...
                        me->SetHealth(me->CountPctFromMaxHealth(me->GetAura(SPELL_WOUNDED)->GetSpellInfo()->GetEffect(EFFECT_0)->BasePoints));
                        for (uint8 i = 0; i < 3; i++)
                            me->GetScheduler().Schedule(Seconds(20), [this, i](TaskContext /*context*/) { me->DealDamage(me, me->CountPctFromMaxHealth(i == 2 ? 26 : 25)); });
                        break;
                    case NPC_TERRIBLY_WOUNDED_PILOT:
                        me->CastSpell(me, SPELL_TERRIBLY_WOUNDED);
                        me->SetHealth(me->CountPctFromMaxHealth(me->GetAura(SPELL_TERRIBLY_WOUNDED)->GetSpellInfo()->GetEffect(EFFECT_0)->BasePoints));
                        for (uint8 i = 0; i < 5; i++)
                            me->GetScheduler().Schedule(Seconds(12), [this, i](TaskContext /*context*/) { me->DealDamage(me, me->CountPctFromMaxHealth(i == 4 ? 11 : 10)); });
                        break;
                    case NPC_CRITICALLY_WOUNDED_PILOT:
                        me->CastSpell(me, SPELL_CRITICALLY_WOUNDED);
                        me->SetHealth(me->CountPctFromMaxHealth(me->GetAura(SPELL_CRITICALLY_WOUNDED)->GetSpellInfo()->GetEffect(EFFECT_0)->BasePoints));
                        for (uint8 i = 0; i < 5; i++)
                            me->GetScheduler().Schedule(Seconds(20), [this, i](TaskContext /*context*/) { me->DealDamage(me, me->CountPctFromMaxHealth(i == 4 ? 6 : 5)); });
                        break;
                    case NPC_MORTALLY_WOUNDED_PILOT:
                        me->CastSpell(me, SPELL_MORTALLY_WOUNDED);
                        me->SetHealth(me->CountPctFromMaxHealth(me->GetAura(SPELL_MORTALLY_WOUNDED)->GetSpellInfo()->GetEffect(EFFECT_0)->BasePoints));
                        me->GetScheduler().Schedule(Seconds(20), [this](TaskContext /*context*/) { me->DealDamage(me, me->CountPctFromMaxHealth(1)); });
                        break; 
                }
            }
            ...
            void HealReceived(Unit* healer, uint32& heal) override
            {
                if (healer->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    if (me->HealthAbovePctHealed(100, heal) && me->GetHealth() < me->GetMaxHealth())
                    {
                        ...
                        me->m_Events.KillAllEvents(true);
                        AddTimedDelayedOperation(5000, [this]()
                        {
                            Position pos;
...
class npc_instructor_bladewind : public CreatureScript
{
    public:
        ...
        bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
        {
            ...
            {
                serpent->AddUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
                /*
                ...
                AddTimedDelayedOperation(serpent->GetSplineDuration(), [this, serpent]() -> void
                {
                    serpent->RemoveUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
...
// Sam the Wise 68128
struct npc_sam_the_wise : public ScriptedAI
{
    npc_sam_the_wise(Creature* creature) : ScriptedAI(creature) { }
    ...
    {
        events.Reset();
        me->AddUnitFlag(UnitFlags(UNIT_FLAG_PACIFIED | UNIT_FLAG_IMMUNE_TO_PC));
        hasTriggered = false;
        hasDefeat = false;
    }
    ...
    void DoAction(int32 actionId) override
    {
        ...
        {
            hasTriggered = true;
            me->RemoveUnitFlag(UnitFlags(UNIT_FLAG_PACIFIED | UNIT_FLAG_IMMUNE_TO_PC));
        }
    }
...
struct npc_plight_general_bunny : public ScriptedAI
{
    npc_plight_general_bunny(Creature* creature) : ScriptedAI(creature) { }
...
// Wild Prowler 56653
struct npc_jf_wild_prowler : public ScriptedAI
{
    npc_jf_wild_prowler(Creature* creature) : ScriptedAI(creature) { }
...
// Wild Stalker 56683
struct npc_jf_wild_stalker : public ScriptedAI
{
    npc_jf_wild_stalker(Creature* creature) : ScriptedAI(creature) { }
...
class npc_lorewalker_cho_stones_could_speak : public CreatureScript
{
    public:
        ...
        struct npc_lorewalker_cho_stones_could_speakAI : public ScriptedAI
        {
            npc_lorewalker_cho_stones_could_speakAI(Creature* creature) : ScriptedAI(creature) { }
        };
...
struct npc_lorewalker_cho_peering_into_past : public ScriptedAI
{
    npc_lorewalker_cho_peering_into_past(Creature* creature) : ScriptedAI(creature) { }
...
class npc_prince_anduin_decision_questgiver : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "It is time to go home, Prince Anduin.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1); // miss gossip_menu_id
            SendGossipMenuFor(player, player->GetGossipTextId(creature), creature->GetGUID());
            return true;
        }

        struct npc_prince_anduin_decision_questgiverAI : public ScriptedAI
        {
            npc_prince_anduin_decision_questgiverAI(Creature* creature) : ScriptedAI(creature) { }
        };
...
// Prince Anduin 56485
struct npc_prince_anduin_decision : public ScriptedAI
{
    npc_prince_anduin_decision(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_prince_anduin_decision_helpers : public ScriptedAI
{
    npc_prince_anduin_decision_helpers(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_jade_forest_boiling_cauldron : public ScriptedAI
{
    npc_jade_forest_boiling_cauldron(Creature* creature) : ScriptedAI(creature) { }
...
struct npc_jade_forest_alliance_barricade : public ScriptedAI
{
    npc_jade_forest_alliance_barricade(Creature* creature) : ScriptedAI(creature) { }
...
class spell_reverse_cast_ride_seat_1 : public SpellScript
{
    ...
    void HandleEffectHit(SpellEffIndex effIndex)
    {
        if (Unit* target = GetHitUnit())
            target->CastSpell(GetCaster(), GetSpellInfo()->GetEffect(effIndex)->BasePoints, true);
    }
...
void AddSC_jade_forest()
{
    ...
    new npc_big_bao();
    RegisterCreatureAI(npc_silverhorn_stag);
    RegisterCreatureAI(npc_forest_huntress);
    RegisterCreatureAI(npc_greenstone_nibbler);
    RegisterCreatureAI(npc_greenstone_gorger);
    RegisterCreatureAI(npc_greenwood_thief);
    RegisterCreatureAI(npc_ivory_starling);
    RegisterCreatureAI(npc_greenwood_trickster);
    RegisterCreatureAI(npc_brittle_greenstone_gorger);
    RegisterCreatureAI(npc_shade_of_ling_heartfist);
    RegisterCreatureAI(npc_jade_guardian);
    RegisterCreatureAI(npc_subjugator_gormal);
    RegisterCreatureAI(npc_orchard_wasp);
    RegisterCreatureAI(npc_ink_tipped_crane);
    RegisterCreatureAI(npc_crane_hunter);
    RegisterCreatureAI(npc_morgrinn_crackfang);
    RegisterCreatureAI(npc_shanze_spiritclaw);
    RegisterCreatureAI(npc_urobi_the_walker);
    RegisterCreatureAI(npc_siftworm);
    RegisterCreatureAI(npc_honeykeeper);
    RegisterCreatureAI(npc_final_doubt);
    RegisterCreatureAI(npc_nurse_shark);
    RegisterCreatureAI(npc_slitherscale_lizard_lord);
    RegisterCreatureAI(npc_shadowfae_trickster);
    RegisterCreatureAI(npc_thunderfist_gorilla);
    // Quest scripts
    new npc_nectarbreeze_farmer();
    RegisterCreatureAI(npc_windward_hatchling);
    RegisterCreatureAI(npc_windward_nest_trigger);
    ...
    RegisterCreatureAI(npc_sam_the_wise);
    RegisterCreatureAI(npc_plight_general_bunny);
    RegisterCreatureAI(npc_jade_forest_boiling_cauldron);
    RegisterCreatureAI(npc_jf_wild_prowler);
    RegisterCreatureAI(npc_jf_wild_stalker);
    RegisterCreatureAI(npc_lorewalker_cho_peering_into_past);
    RegisterCreatureAI(npc_prince_anduin_decision);
    RegisterCreatureAI(npc_prince_anduin_decision_helpers);
    RegisterCreatureAI(npc_jade_forest_alliance_barricade);
    RegisterCreatureAI(npc_jade_forest_playful_colored_serpent);
    RegisterCreatureAI(npc_jade_forest_instant_message_camera_bunny);

    new go_jade_forest_keg_of_metal_brew();

    //new spell_q30063();
    new spell_q29637();
    RegisterSpellScript(spell_jade_forest_smoked_blade);
    RegisterSpellScript(spell_jade_forest_summon_metal_brewmaster);
    RegisterSpellScript(spell_summon_boiling_cauldron);
    RegisterSpellScript(spell_jade_forest_lorewalker_cho_monolith);
    RegisterSpellScript(spell_jade_forest_mouthwatering_brew_launcher);
    RegisterSpellScript(spell_jade_forest_nazgrims_flare_gun);
    RegisterSpellScript(spell_reverse_cast_ride_seat_1);

    RegisterAuraScript(spell_cho_teapot_master);
    RegisterAuraScript(spell_jade_forest_signal_flare_initialize);
