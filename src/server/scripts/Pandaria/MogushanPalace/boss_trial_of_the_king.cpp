    ...
    public:
        ...
        struct npc_xian_the_weaponmaster_trigger_AI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (!m_bEventGo)
                    if (Player* itr = me->SelectNearestPlayer(35.0f)) // Launch the speech when a player steps close to Xin the Weaponmaster
                    {
                        m_bEventGo = true;
        ...
        struct boss_ming_the_cunning_AI : public BossAI
        {
            ...
            void EnterEvadeMode(EvadeReason w) override
            {
              ...
              /*  Movement::MoveSplineInit init(me);
                ...
                ObjectGuid tempGUID = me->GetGUID();
                AddTimedDelayedOperation(11 * TimeConstants::IN_MILLISECONDS, [this, tempGUID]() -> void
                    {
                        me->StopMoving();
    ...
    public:
        ...
        struct npc_whirling_dervish_AI : public ScriptedAI
        {
            ...
            void HandleCircularMovement()
            {
                ...
                //newZ = me->GetMap()->GetHeight(newX, newY, newZ);
                newZ = me->GetMap()->GetHeight(me->GetPhaseShift(), newX, newY, z, true);
            ...
            void DoAction(int32 actionId) override
            {
                switch (actionId)
                {
                    case ACTION_ENCOURAGE:
                    {
                        ...
                        Movement::MoveSplineInit init(me);                        
                        init.MoveTo(x, y, me->GetMap()->GetHeight(me->GetPhaseShift(),x, y, me->GetPositionZ()));
                        init.Launch();
...
class spell_shockwave_kuai : public AuraScript
{
    ...
    {
        if (GetCaster() && GetCaster()->ToCreature() && !GetCaster()->HasUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC)))
            GetCaster()->ToCreature()->InterruptSpell(CURRENT_CHANNELED_SPELL);
...
class spell_shockwave_kuai_eff : public SpellScript
{
    ...
    {
        if (GetCaster() && GetCaster()->HasUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC)))
        ...
        class spell_shockwave_kuai_SpellScript : public SpellScript
        {
            ...
            void HandleSpellcast()
            {
                ...
                if (GetHitUnit() && GetCaster() && GetCaster()->ToCreature())
                    GetCaster()->ToCreature()->CastSpell(GetCaster()->GetAngle(GetHitUnit()), SPELL_SHOCKWAVE);
            }
        ...
        class spell_saurok_help_call_SpellScript : public SpellScript
        {
            ...
            {/*
                if (GetHitUnit())
                {
                    ...
                    GetHitUnit()->GetMotionMaster()->MovePoint(0, x, y, GetHitUnit()->GetMap()->GetHeight(0,x, y, GetHitUnit()->GetPositionZ()));
