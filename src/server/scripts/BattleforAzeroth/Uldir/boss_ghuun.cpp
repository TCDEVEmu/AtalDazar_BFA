        ...
        {            
            if (Unit* matrixHolder = SelectTarget(SELECT_TARGET_RANDOM, 0, 25.0f, true))
            {
                ...
                if (matrixHolder->HasAura(SPELL_POWER_MATRIX))
                    me->GetThreatManager().addThreat(matrixHolder, 1000.0f, SpellSchoolMask::SPELL_SCHOOL_MASK_NORMAL);
            }
...
private:
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        ...
        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0 , 100.0f, true))
        {
            ...
            me->GetThreatManager().addThreat(target, 1000.0f, SpellSchoolMask::SPELL_SCHOOL_MASK_NORMAL);
