    ...
    public:
        ...
        struct npc_wind_lord_meljarak_introAI : public ScriptedAI
        {
            ...
            {/*
                ...
                for (auto&& itr : Familiars)
                {
                    itr->AddUnitFlag(UNIT_FLAG_NON_ATTACKABLE); // it`s only if were crash/reboot of server
                    itr->SetReactState(REACT_PASSIVE);
...
void AddSC_heart_of_fear()
{
    ...
    RegisterCreatureAI(npc_zephyr_tornado);
    new npc_heart_of_fear_kazrik();

    RegisterSpellScript(spell_searing_slash);
    RegisterSpellScript(spell_amber_volley);
}
